...
def get_estimated_value(year, month, day):
    ...
    uranus = floor((chart.getObject(const.URANUS).lon / 30 + 4) % 12)
    neptune = floor((chart.getObject(const.NEPTUNE).lon / 30 + 4) % 12)
    pluto = floor((chart.getObject(const.PLUTO).lon / 30 + 4) % 12)
    if sun==0:
        sun=12
    if moon==0:
        moon=12
    ...
    uranus_degrees = chart.get(const.URANUS).lon % 30.0
    neptune_degrees = chart.get(const.NEPTUNE).lon % 30.0
    pluto_degrees = chart.get(const.PLUTO).lon % 30.0
    



    test_data = [sun, moon, mercury, venus, mars, jupiter, saturn, uranus, neptune, pluto]
    test_data_degrees = [sun_degrees, moon_degrees, mercury_degrees, venus_degrees, mars_degrees, jupiter_degrees, saturn_degrees, uranus_degrees, neptune_degrees, pluto_degrees]


    planets = ["Sun", "Moon", "Mercury", "Venus", "Mars", "Jupiter", "Saturn", "Uranus", "Neptune", "Pluto"]
    exalted = {planets[0]: 4, planets[1]: 5, planets[2]: 9, planets[3]: 3, planets[4]: 1, planets[5]: 7, planets[6]: 10, planets[7]: 11, planets[8]: 7, planets[9]: 8}
    ruling = {planets[0]: [8], planets[1]: [7], planets[2]: [6, 9], planets[3]: [5, 10], planets[4]: [4, 11], planets[5]: [12, 3], planets[6]: [1, 2], planets[7]:[2], planets[8]: [3], planets[9]: [11]}
    exalted_degrees = {planets[0]: 19, planets[1]: 3, planets[2]: 15, planets[3]: 27, planets[4]: 28, planets[5]: 15, planets[6]: 21}
    ...
    exalted_degrees_values = list(exalted_degrees.values())
    estimated_value = 0
    for i in range(0, len(test_data)):
        if test_data[i] == exalted_values[i]:
            estimated_value += EXALTED_VALUE
            summa_cumma_latte[i] += EXALTED_VALUE
            if i <= 6 and int(test_data_degrees[i]) == exalted_degrees_values[i]:
                estimated_value += 3.0
                summa_cumma_latte[i] += 3.0
    ...
    return [round(estimated_value, 1), sign_list, summa_cumma_latte, test_data_degrees]
